...
import {
  ...
  Home, FolderCode, BookOpen, User, Sparkles, Plane, ScrollText
} from 'lucide-react';
...
const Header = () => {
  ...
  const navLinks = [
    ...
    { name: '日志', href: '/logs', icon: ScrollText, desc: '最近动态' },
  ...
  return (
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 ${
          ...
          scrolled || mobileMenuOpen
            ? 'h-16 bg-slate-50/75 backdrop-blur-xl border-b border-slate-200/80 shadow-[0_10px_30px_rgba(15,23,42,0.08)]' 
            : 'h-20 bg-slate-50/55 backdrop-blur-sm border-b border-transparent'
        }`}
      >
        <div className="container mx-auto h-full flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3 group cursor-pointer">
            <div className="relative">
              ...
              <div className="relative w-10 h-10 rounded-xl flex items-center justify-center shadow-lg shadow-[0_12px_30px_rgba(59,130,246,0.18)] group-hover:scale-110 group-active:scale-95 transition-all duration-300">
                <DynamicLogo size={40} showText={false} />
              </div>
            </div>
            <div className="flex flex-col">
              <div className="text-lg font-black tracking-normal text-ui-text leading-tight">
                <span className="text-brand">Just</span> Potato
              </div>
              <div className="text-[9px] text-slate-500 font-bold tracking-[0.4em] uppercase opacity-40">
                Creative Tech
              </div>
            </div>
          </div>
          
          <div className="hidden md:flex items-center gap-1 p-1.5 bg-white/70 rounded-2xl border border-slate-200/70">
              ...
              return (
                <a 
                  ...
                  className={`relative flex items-center gap-2 px-4 py-2 text-sm font-bold transition-all duration-300 z-10 rounded-xl ${
                    hoveredIndex === index ? 'text-brand' : 'text-slate-500 hover:text-slate-900'
                  }`}
                >
                  <Icon size={16} strokeWidth={2.5} className={hoveredIndex === index ? 'animate-bounce-subtle' : ''} />
                  {link.name}
                  {hoveredIndex === index && (
                    <div className="absolute inset-0 bg-white/90 shadow-sm border border-slate-200/80 rounded-xl -z-10 animate-in fade-in zoom-in-95 duration-200" />
                  )}
                </a>
              );
            })}
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <div className="hidden lg:flex items-center gap-3 px-3 py-1.5 rounded-xl bg-ui-border/20 text-ui-text/40 text-xs border border-transparent hover:border-brand/20 transition-all cursor-text group">
              ...
              <kbd className="hidden xl:inline-flex h-5 items-center gap-1 rounded border border-slate-200 bg-white px-1.5 font-mono text-[10px] font-medium opacity-100">
                K
              </kbd>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button className="hidden sm:flex items-center gap-2 rounded-xl bg-brand px-5 py-2.5 text-sm font-bold text-white hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-[0_12px_30px_rgba(59,130,246,0.18)] group">
      ...
      <div 
        className={`fixed inset-0 z-[950] bg-white md:hidden transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${
            ...
            {navLinks.map((link, idx) => {
              ...
              return (
                <a 
                  key={link.name} 
                  href={link.href} 
                  className={`group flex items-center gap-5 p-4 rounded-2xl bg-ui-border/5 border border-slate-200/10 hover:border-brand/30 transition-all duration-500 ${
                    mobileMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-12 opacity-0'
                  }`}
                  style={{ transitionDelay: `${idx * 80}ms` }}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <div className="w-12 h-12 rounded-xl bg-white shadow-sm border border-slate-200/20 flex items-center justify-center text-ui-text/40 group-hover:text-brand group-hover:border-brand/20 transition-all">
                    <Icon size={24} strokeWidth={1.5} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xl font-bold text-ui-text group-hover:text-brand transition-colors">{link.name}</span>
                    <span className="text-xs text-slate-500 font-medium">{link.desc}</span>
                  </div>
             ...
             <div className="p-5 rounded-3xl bg-gradient-to-br from-brand/10 to-transparent border border-brand/10 relative overflow-hidden group">
               <div className="relative z-10 flex items-center justify-between">
                 <div>
                   ...
                   <p className="text-[10px] text-slate-500">订阅我们获取更新</p>
                 </div>
                 <button className="px-4 py-2 bg-brand text-white text-xs font-bold rounded-lg shadow-[0_12px_30px_rgba(59,130,246,0.18)] shadow-md">立即订阅</button>
               </div>
